...
class LLMStrikethroughEdit(BaseModel):
    line_hex: str
    substring_text: str|None

class LLMInsertEdit(BaseModel):
    above_line_hex: str
    insert_md: str

class EditsList(BaseModel):
    strikethrough_edits: list[LLMStrikethroughEdit]
    insert_edits: list[LLMInsertEdit]
...
def insert_page_img_and_text(page: fitz.Page, out_page: fitz.Page, pix_start_y: float, split_y: float, y_offset: float):
...
def multiple_split_edits(
        page: fitz.Page,
        strikethroughs: list[StrikethroughEdit],
...
table { border-collapse: collapse; font-size: 1em; border: 1px solid #000; }
...
"""
    ...
    inserts = sorted(inserts, key = lambda edit: edit.y)

    #determine split heights needed for each insert
    split_heights = []
    for insert_edit in inserts:
        split_y, insert_md = insert_edit.y, insert_edit.insert_md
        ...
        while scale<1.:
            ...
            # print(text_rect)
            # print(insert_html)
            spare_height, scale = new_page.insert_htmlbox(text_rect, insert_html, css = md_css)
            if scale<1.:
    ...
    strikethroughs = sorted(strikethroughs, key = lambda edit: edit.y)
    ...
    while insert_ind<len(inserts) or strikethrough_ind<len(strikethroughs):
        if strikethrough_ind < len(strikethroughs) and (
                insert_ind == len(inserts) or strikethroughs[strikethrough_ind].y<inserts[insert_ind].y
        ):
            strikethrough_edit = strikethroughs[strikethrough_ind]
            out_page.add_line_annot((strikethrough_edit.x0-2, strikethrough_edit.y+y_offset), (strikethrough_edit.x1+2, strikethrough_edit.y+y_offset))
            strikethrough_ind+=1

        else:
            split_height = split_heights[insert_ind]
            split_y, insert_md = inserts[insert_ind].y, inserts[insert_ind].insert_md
            insert_html = markdown(insert_md, extensions = ['extra'])


            if split_y-pix_start_y>1.:
                # insert the image and text for the portion above the split
                insert_page_img_and_text(page, out_page, pix_start_y, split_y, y_offset)
            ...
            spare_height, scale = out_page.insert_htmlbox(text_rect, insert_html, css=md_css, scale_low=.9)
            if spare_height == -1:
    ...
    if page.rect.height - pix_start_y > 1.:
        # insert the image and text for the portion below the last split
        insert_page_img_and_text(page, out_page, pix_start_y, page.rect.height, y_offset)
...
def get_page_lines(page: fitz.Page, line_tolerance: float = 0.8) -> list[list[float, float, str, list]]:
    words = page.get_text('words')
    ...
    for word in sorted_words:
        ...
        if overlap > line_tolerance:
            ...
            leading_spaces = max(0, round(curr_line_words[0][0] / average_char_w)) if average_char_w > 0 else 0
            ...
            lines[-1].append(curr_line_words)
